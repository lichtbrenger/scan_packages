...
def determine_os_database():
    ...
    if subprocess.call(['which', 'yum']) == 0:
        ...
        os_dict['queries'] = ['select name,version from rpm',
                              'select name from rpm']

    if not os_dict['operating system']:
        logger.error('Operating system and corresponding'
...
def query_db(conn, query):
    ...
    try:
        ...
        if not rows:
            logger.error('could not retrieve any data using'
                         'the following query: {}.'
                         .format(query))
            return
